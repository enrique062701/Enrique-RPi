...
"""
###THIS IS AN EDITED FILE USED FOR MOTORS WITH NO ENCODER FEEDBACK
...
class Motor_Control:
    ...
    def __init__(self, server_ip_addr = None, cm_per_turn = 0.0635, stop_switched=True, msipa_cache_fn = None, verbose = False, name='not named'): #0.254 for larger screw
...
########################################################################################################
    ...
    def send_text(self, text, timeout:int=None, receive=True) -> str:
        ...
        #####JACKSON NOTE: It seems that without the encoder, a time delay is needed for commands
...
#-------------------------------------------------------------------------------------------
    ...
    @property
    def instant_position(self):
        """ Returns the instantaneous position in units of cm"""

        resp = self.send_text('IP') ####SWAPPED IE WITH IP
        pos = int(resp[5:]) /self.__stepsPerRev * self.cm_per_turn
        return (pos)
...
#-------------------------------------------------------------------------------------------
    ...
    #        resp = self.send_text('EP')  # Ask for encoder position
            ...
            try:
    #            pos = int(resp[5:])  /self.__stepsPerRev * self.cm_per_turn
                pos1 = int(resp1[5:])  /self.__stepsPerRev * self.cm_per_turn
                return pos1
    ...
    #                return round(pos,4)
                ...
                if '*' in resp1:
                    ...
                    print('Invalid response for %s : motor %s ; encoder %s' %(self.name, resp1) )
                    retry_count += 1
                    if retry_count == 3:
...
#-------------------------------------------------------------------------------------------
    ...
    def current_step(self):
        ...
        '''
        resp = self.send_text('DI') ###CHANGED EP WITH DI
        return int(resp[5:])
...
#-------------------------------------------------------------------------------------------
    # @property
    def set_zero(self): ###CHANGED FOR NO ENCODER
        """ Set motor position and encoder position to zero
        """
        self.send_text('SP0')  # Set position to zero
        resp = self.send_text('IP')
        if int(resp[5:]) == 0: print (self.name + '-motor: Set current position to zero')
        else : print (self.name + '-motor: Fail to set current position to zero')
...
#	 		print('enable ', sep='', end='', flush=True)
    ...
    @motor_position.setter #JACKSON MADE THIS TO CREATE A JOG COMMAND. To be more accurate, this is not technically a motor_position.setter but I felt no need to create a motor_jog_position as this info is meaningless (so the next best thing is motor_position)
    def jog(self, pos):
        self.send_text('FL'+str(self.cm_to_steps(pos)))
...
#===============================================================================================================================================
# standalone testing for Epics command:
