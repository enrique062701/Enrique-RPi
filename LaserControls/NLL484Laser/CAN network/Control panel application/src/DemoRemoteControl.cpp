   ...
   if(RMTCTRLERR_OK == Code)
   {
      switch(this->type)
      {
       case Edit:
         ...
         if(evenfocused ||
             (   this->edit->Focused() == false
              && (!this->SetButton || this->SetButton->Focused() == false)
              && (!this->NVButton  || this->NVButton->Focused() == false)
             )
            ) this->edit->Text = regval;
         break;
       case Combo:
...
double d;
   ...
   switch(this->type)
   {
    case Edit:
      ...
      if(numstrlen)
      {
         d = strtod(tmp, NULL) +  inc * incmultiplier;
         edit->Text = d;

         Code = rcSetRegFromDouble(this->DevName.c_str(), this->RegName.c_str(),
                                    d);
      }
      this->Update(1);
      break;
...
double ParameterValue;
   ...
   Code = rcConnect(OptionsForm->PortRG->ItemIndex, OptionsForm->COMUpDown->Position);
